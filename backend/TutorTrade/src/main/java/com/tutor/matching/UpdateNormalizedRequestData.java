...
/**
 * Class receives DynamoDB events containing request object, checks that the changes represent an
  ...
  public Void handleRequest(DynamodbEvent dynamodbEvent, Context context) {
    for (DynamodbEvent.DynamodbStreamRecord record : dynamodbEvent.getRecords()) {
      ...
      if (oldRequestRecord.get("status").getS().equals("COMPLETED")
        ...
        System.out.printf(
            "Aborted. Old: %s\n New: %s \n",
            oldRequestRecord.get("status").getS(), newRequestRecord.get("status").getS());
        return null;
      }
      ...
      // check if model data already exists. If it does, turn it into a
      // request data list, so we can append new data
      ...
      // add new data, re-serialize and return updated data to s3,
